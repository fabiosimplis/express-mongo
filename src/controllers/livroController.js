...
class LivroController {
  static async listarLivros(req, res) {
    try {
      ...
      res.status(500).json({
        message: `${erro.message} - falha na requisição ao buscar os livros`,
      });
    }
  }
  ...
  static async listarLivroPorId(req, res) {
    try {
      ...
      res.status(500).json({
        message: `${erro.message} - falha na requisição ao buscar o livro`,
      });
    }
  }
  ...
  static async deletarLivroPorId(req, res) {
    try {
      ...
      res.status(500).json({
        message: `${erro.message} - falha exclusão do livro`,
      });
    }
  }
}
